...
export function TaskForm({
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (!id || !optimistic || !mostLikely || !pessimistic) {
			return;
		}

		onSubmit({
			id,
			optimistic: Number.parseFloat(optimistic),
			most_likely: Number.parseFloat(mostLikely),
			pessimistic: Number.parseFloat(pessimistic),
			dependencies,
		});
	};

	const addDependency = () => {
		if (dependencyInput && !dependencies.includes(dependencyInput)) {
			setDependencies([...dependencies, dependencyInput]);
			setDependencyInput("");
		}
	};
	...
	return (
		<Card className="border-2 border-primary">
			<CardHeader>
				...
				<CardDescription>
					Enter task details with time estimates in days
				</CardDescription>
			</CardHeader>
			<CardContent>
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<div className="space-y-2">
							<Label htmlFor="optimistic">Optimistic Time (days)</Label>
							<Input
								id="optimistic"
								type="number"
								step="0.1"
						...
						<div className="space-y-2">
							<Label htmlFor="mostLikely">Most Likely Time (days)</Label>
							<Input
								id="mostLikely"
								type="number"
								step="0.1"
						...
						<div className="space-y-2">
							<Label htmlFor="pessimistic">Pessimistic Time (days)</Label>
							<Input
								id="pessimistic"
								type="number"
								step="0.1"
					...
					<div className="space-y-2">
						...
						<div className="flex gap-2">
							<Input
								...
								placeholder="Enter task ID that must complete first"
								onKeyDown={(e) => {
									if (e.key === "Enter") {
										e.preventDefault();
										addDependency();
							...
							/>
							<Button type="button" onClick={addDependency} variant="secondary">
								Add
							</Button>
						</div>
